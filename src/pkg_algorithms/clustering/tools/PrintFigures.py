...
class PrintFigures:
    ...
    def printScatter_Color(self, points, label, k, name):
        ...
        if points.shape[1] == 2:
            ...
            for i in range(len(points)):
                index = int(label[i])
                if index == 0:
                    ax.scatter(points[i][0], points[i][1], points[i][2], color=colors[index], edgecolors='b',
                               marker='*')
                else:
    ...
    def printScatter_Color_Marker(self, points, label):
        ...
        for j in range(cNum):
            ...
            plt.scatter(points[idx, 0], points[idx, 1], color=colors[j % len(colors)], label=('C' + str(j)),
                        marker=markers[j % len(markers)], s=20)
    ...
    def printScatterCircle(self, points, label, rs):
        ...
        for j in range(cNum + 1):
            ...
            print("idx:", idx)
            plt.scatter(points[idx, 0], points[idx, 1], color=colors[j % len(colors)], label=('C' + str(j)),
                        marker=markers[j % len(markers)], s=30)
            # plt.xlabel('x'), plt.ylabel('y')
        for i in range(len(points)):
            print("rs", "i:", rs[i])
        ...
        for i in range(length):
            ...
            for j in arg_sort[:5]:
                if i == j:
                    ...
                    plt.plot([points[cores[i]][0], points[cores[j]][0]], [points[cores[i]][1], points[cores[j]][1]],
                             color='r')
                # s = ''+str(a)+'+'+str(b) +"+"+ str(round(neighbor[i][2],1))
                s = str(round(GD[i][j], 2))
                plt.text((points[cores[i]][0] + points[cores[j]][0]) / 2,
                         (points[cores[i]][1] + points[cores[j]][1]) / 2, s)
        for i in range(length):
